...
// Aprašyk sumWithDiscount(a, b, discount) JSDoc: du privalomi skaičiai, vienas pasirenkamas su numatytąja reikšme.Grąžina skaičių ir pateikia bent vieną @example.
 /** 
  * Sudeda du skaičius, gauna sumą ir grąžina galutinę sumą po nuolaidos
  * @param {number} a - Pirmas skaičius
  * @param {number} b - Antras skaičius
  * @param {number} discount - duota nuolaida
  * @returns {number} - suma po nuolaidos
 */
...
// Aprašyk JSDoc greet({ name, age }).name privaloma, age pasirenkama.Išmeta klaidą, jei name tuščias.Grąžina tekstą.
 ...
 /** 
  * Sugeneruoja pasveikinimo tekstą pagal pateiktą vardą ir (neprivalomai) amžių.
  * @param {name: string, age?: number} params - Vartotojo informacija
  * @param {string} params.name - Vartotojo vardas(privalomas)
  * @param {number} [params.age] - Vartotojo amžius(neprivalomas)
  * @returns {string} - Sugeneruotas pasveikinimo tekstas
  * @throws {Error} - Jei name yra tuščias arba nepateiktas
 */
